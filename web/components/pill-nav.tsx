...
export default function PillNav({ logo, logoAlt, items, activeHref, className = "" }: PillNavProps) {
  ...
  return (
    <nav ref={navRef} className={`pill-nav ${className}`}>
      {logo && <img src={logo || "/placeholder.svg"} alt={logoAlt || "Logo"} className="h-8 w-8" />}
      <div className="flex items-center gap-2">
        {items.map((item) => (
          <Link key={item.href} href={item.href}>
            <span className={`pill-nav-item ${activeHref === item.href ? "active" : ""}`}>{item.label}</span>
          </Link>
        ))}
      </div>
      <a
